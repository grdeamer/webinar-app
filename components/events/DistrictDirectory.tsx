...
export default function DistrictDirectory({ items }: { items: DistrictDirectoryItem[] }) {
  ...
  return (
    <section data-district-directory className="min-w-0 overflow-hidden rounded-3xl border border-white/10 bg-[#09111f]/90 shadow-[0_28px_80px_rgba(0,0,0,.28)]">
      ...
      <div className="grid min-h-0 md:min-h-[340px] md:grid-cols-[minmax(260px,.8fr)_minmax(0,1.2fr)]">
        <nav aria-label="District tree" className="max-h-[65vh] min-w-0 overflow-y-auto overscroll-contain border-b border-white/10 p-3 sm:p-4 md:max-h-none md:border-b-0 md:border-r">
          <ul className="space-y-1">{renderBranch(null)}</ul>
        </nav>
        <div className="flex min-w-0 items-center p-5 sm:p-6 md:p-9">
          {selected ? <div className="w-full">
            ...
            <h3 className="mt-3 break-words text-2xl font-semibold tracking-[-.03em] text-white sm:text-3xl">{selected.title}</h3>
