  ...
  try {
    ...
    const [liveStateResult, agendaResult] = await Promise.all([
      ...
      supabaseAdmin
        ...
        .select(
          "id,title,description,speaker,speaker_title,speaker_bio,speaker_photo_url,speakers,show_session_details,show_speaker_photo,show_speaker_bio,resources,show_resources,district_lookup_enabled,icon_key,track,location,start_at,end_at,sort_index,status,button_text,button_url,is_visible,updated_at"
        )
        .eq("event_id", event.id)
        .eq("is_visible", true)
