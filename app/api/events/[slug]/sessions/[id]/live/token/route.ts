...
export async function POST(
  ...
  if (!experience.access.canView) {
    const status = experience.access.reason === "login_required" ? 401 : 403
    return json({ error: "You do not have access to this session" }, status)
  }

  if (experience.delivery.kind !== "livekit") {
    return json({ error: "This session is not a LiveKit session" }, 400)
  }
  ...
  const attendeeEmail =
    typeof viewer.email === "string" && viewer.email.trim()
      ? viewer.email.trim().toLowerCase()
      : null

  const attendeeName = attendeeEmail || "Attendee"

  const identity = attendeeEmail || `attendee-${crypto.randomUUID()}`
  const displayName = attendeeName
