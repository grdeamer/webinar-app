...
export default function EventEmailGate(props: { slug: string; eventTitle?: string | null }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      window.location.href = `/events/${slug}/lobby`
    } catch (err: any) {
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.14),transparent_25%),linear-gradient(135deg,rgba(15,23,42,0.9),rgba(2,6,23,0.95))] p-6 shadow-2xl shadow-black/25">
      <div className="text-xs uppercase tracking-[0.2em] text-white/45">Attendee access</div>
      <h2 className="mt-3 text-2xl font-bold">Enter your event email</h2>
      <p className="mt-3 text-sm leading-6 text-white/68">
        {eventTitle ? (
          <>
            Use the email assigned to <span className="font-semibold text-white">{eventTitle}</span>.
          </>
        ) : (
          "Use the email assigned to your event registration."
        )}{" "}
        We will route you to your personalized lobby and only show sessions assigned to you.
      </p>
      ...
      <form onSubmit={submit} className="mt-6 space-y-3">
        ...
        >
          {busy ? "Checking access..." : "Continue to my lobby"}
        </button>
      </form>

      <div className="mt-5 grid gap-3 text-xs text-white/50 sm:grid-cols-3">
        <div className="rounded-2xl border border-white/10 bg-black/20 px-3 py-3">Assigned session access</div>
        <div className="rounded-2xl border border-white/10 bg-black/20 px-3 py-3">Embedded playback when available</div>
        <div className="rounded-2xl border border-white/10 bg-black/20 px-3 py-3">Agenda + materials in one place</div>
      </div>
    </section>
  )
}
