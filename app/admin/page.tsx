...
type MembershipRow = { event_id: string; role: EventTeamRole; feature_permissions: string[] | null }
type ActivityRow = { session_id: string; current_path: string; updated_at: string }
...
export default async function AdminDashboardPage() {
  ...
  const { data: membershipData } = isGlobalAdmin
    ? { data: [] as MembershipRow[] }
    : await supabaseAdmin
        .from("event_team_members")
        .select("event_id,role,feature_permissions")
        .eq("user_id", user.id)
        .eq("is_active", true)
  const memberships = (membershipData ?? []) as MembershipRow[]
  const allowedEventIds = memberships.map((membership) => membership.event_id)
  const noEventId = "00000000-0000-0000-0000-000000000000"

  // eslint-disable-next-line react-hooks/purity
  const now = Date.now()
  const cutoff = new Date(now - 45_000).toISOString()
  let eventsQuery = supabaseAdmin.from("events").select("id,title,slug,start_at,lifecycle_stage").neq("lifecycle_stage", "archived").order("start_at", { ascending: true, nullsFirst: false })
  let liveQuery = supabaseAdmin.from("event_live_state").select("event_id,is_live,destination_session_id,updated_at").eq("is_live", true).order("updated_at", { ascending: false })
  if (!isGlobalAdmin) {
    const scopedIds = allowedEventIds.length ? allowedEventIds : [noEventId]
    eventsQuery = eventsQuery.in("id", scopedIds)
    liveQuery = liveQuery.in("event_id", scopedIds)
  }
  const [eventsResult, liveResult] = await Promise.all([eventsQuery, liveQuery])
  ...
  const activityResult = sessionIds.length
    ? await supabaseAdmin.from("attendee_activity").select("session_id,current_path,updated_at").in("session_id", sessionIds).gte("updated_at", cutoff).order("updated_at", { ascending: false })
    : null
  ...
  const actions = upcoming.flatMap((event) => {
    ...
    const membership = memberships.find((item) => item.event_id === event.id)
    const canManagePeople = isGlobalAdmin || Boolean(membership && featuresForEventRole(membership.role, membership.feature_permissions).includes("people"))
    return readiness.missing && canManagePeople ? [{ event, ...readiness }] : []
  })
  const canUseFeature = (eventId: string, feature: "producer_room") => {
    if (isGlobalAdmin) return true
    const membership = memberships.find((item) => item.event_id === eventId)
    return Boolean(membership && featuresForEventRole(membership.role, membership.feature_permissions).includes(feature))
  }
    ...
    <div className="global-editorial-page mx-auto max-w-[1500px] space-y-5 pb-8">
      <header data-dashboard-tour="welcome" className="dashboard-command-header flex flex-col gap-6 px-1 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <MissionControlTour userId={user.id} />
          ...
          {isGlobalAdmin ? <Link href="/admin/events/new" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-indigo-300/35 bg-gradient-to-r from-blue-600 to-violet-600 px-5 text-sm font-semibold shadow-[0_12px_34px_rgba(82,83,255,.2)] hover:brightness-110"><Plus size={17} />Create event</Link> : null}
        </div>
      </header>

      <section data-dashboard-tour="live" className="dashboard-command-panel relative overflow-hidden rounded-[28px] border border-blue-300/15 bg-[radial-gradient(circle_at_82%_18%,rgba(83,82,255,.16),transparent_34%),linear-gradient(135deg,rgba(10,18,39,.98),rgba(5,10,25,.96))] shadow-[0_30px_90px_rgba(0,0,0,.28)]">
          ...
          <div className="relative grid min-h-[360px] gap-8 p-7 sm:p-9 xl:grid-cols-[1.15fr_.85fr] xl:items-stretch xl:p-10">
            <div className="flex min-w-0 flex-col justify-between">
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                {canUseFeature(liveEvent.id, "producer_room") ? <Link href={`/admin/events/${liveEvent.id}/producer/room`} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-violet-600 px-6 text-sm font-semibold shadow-[0_16px_35px_rgba(77,83,255,.25)] hover:brightness-110">Open Producer Room<ArrowUpRight size={16} /></Link> : null}
      ...
      <section data-dashboard-tour="attention" className="dashboard-command-panel rounded-2xl border border-white/10 bg-[#080e1d]/72 px-6 py-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.12fr_.88fr]">
        <section data-dashboard-tour="events" className="dashboard-command-panel rounded-2xl border border-white/10 bg-[#080e1d]/76 p-6">
        ...
        <section data-dashboard-tour="activity" className="dashboard-command-panel rounded-2xl border border-white/10 bg-[#080e1d]/70 p-6">
          <div className="flex items-center justify-between gap-3 border-b border-white/[.08] pb-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.18em] text-white/70"><Activity size={16} />Recent activity</div>{isGlobalAdmin ? <Link href="/admin/activity" className="text-xs font-semibold text-blue-300">View all activity</Link> : null}</div>
      ...
      <section className="flex flex-col gap-4 rounded-2xl border border-white/[.08] bg-[#080e1d]/52 px-6 py-4 lg:flex-row lg:items-center">
        <div className="flex shrink-0 items-center justify-between gap-5 lg:pr-5"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.18em] text-white/55"><ShieldCheck size={16} />Platform health</div>{isGlobalAdmin ? <Link href="/admin/health" className="text-xs font-semibold text-blue-300 lg:hidden">View status</Link> : null}</div>
        ...
        {isGlobalAdmin ? <Link href="/admin/health" className="hidden shrink-0 text-xs font-semibold text-blue-300 lg:block">View status<ArrowUpRight size={13} className="ml-1 inline" /></Link> : null}
      </section>
    </div>
  )
}
