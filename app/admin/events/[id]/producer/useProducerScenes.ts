...
function clonePreviewBlock(block: PreviewBlock): PreviewBlock {
  return {
    ...
    blur: block.blur ?? 0,
    glow: block.glow ?? 0,
    glowColor: block.glowColor ?? "#7dd3fc",
    ...
    assignedTrackSid: block.assignedTrackSid ?? null,
    placeholderEmoji: block.placeholderEmoji ?? (block.type === "camera-slot" ? "👤" : null),
    placeholderLabel: block.placeholderLabel ?? (block.type === "camera-slot" ? "Camera Slot" : null),
    placeholderSubLabel: block.placeholderSubLabel ?? (block.type === "camera-slot" ? "Assign presenter or attendee" : null),
    placeholderStyle: block.placeholderStyle ?? (block.type === "camera-slot" ? "branded" : undefined),
    ...
    animationType: block.animationType ?? "none",
    animationProgress: block.animationProgress ?? 1,
  }
}
