    ...
    tone === "green"
      ...
      : tone === "violet"
        ...
        : tone === "sky"
          ...
          : "border-white/6 bg-white/[0.045] text-white/42"

  return (
    <div className={`rounded-full border px-3 py-1 text-[9px] font-black uppercase tracking-[0.14em] ${toneClass}`}>
...
function SceneMemoryBadge({
  ...
  return (
    <div
      className={[
        "flex items-center gap-2 rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] transition-all duration-300",
        active
...
function StatusChip({
  ...
  return (
    ...
    >
      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl border border-white/6 bg-white/[0.06] text-white/62">
        {icon}
      </span>

      <div className="min-w-0">
        <div className="text-[9px] font-black uppercase tracking-[0.14em] text-white/34">
          {label}
        </div>
...
export default function ScenesStatusPanel({
  ...
  return (
    <div className="relative overflow-hidden rounded-[26px] border border-white/6 bg-[linear-gradient(180deg,rgba(11,15,26,0.94),rgba(5,8,16,0.985))] p-3 shadow-[0_18px_54px_rgba(0,0,0,0.24),inset_0_1px_0_rgba(255,255,255,0.03)]">
      <div className="mb-3 rounded-[26px] border border-white/6 bg-[linear-gradient(180deg,rgba(255,255,255,0.045),rgba(255,255,255,0.02))] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_14px_34px_rgba(0,0,0,0.16)]">
      ...
      <div className="flex flex-wrap items-center gap-2.5 rounded-[26px] border border-white/6 bg-white/[0.045] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_14px_34px_rgba(0,0,0,0.16)]">
        <input
          value={sceneName}
          onChange={(e) => onSceneNameChange(e.target.value)}
          placeholder={selectedSceneId ? "Update selected scene" : "Scene name"}
          className="min-w-[220px] rounded-2xl border border-white/6 bg-white/[0.045] px-3 py-2 text-sm text-white outline-none ring-0 placeholder:text-white/28 focus:border-violet-300/25"
        />
        ...
        <div className="flex items-center gap-2 rounded-2xl border border-emerald-300/14 bg-emerald-400/8 px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] text-emerald-100/62 shadow-[0_0_20px_rgba(110,231,183,0.06)]">
          <CheckCircle2 size={13} />
          Scene Recall Ready
        </div>

        <div className="flex items-center gap-2 rounded-2xl border border-violet-300/14 bg-violet-400/8 px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] text-violet-100/58 shadow-[0_0_18px_rgba(168,85,247,0.08)]">
          <Radio size={13} />
          Cue Stack Synced
        </div>
        ...
        {stageState?.screen_share_participant_id ? (
          <button
            ...
            className="rounded-xl border border-white/18 bg-white/[0.06] px-3 py-1.5 text-[11px] font-black uppercase tracking-[0.14em] text-white transition"
          >
            Clear screen
          </button>
        ) : null}
      ...
      {selectedSceneId ? (
        <div className="mt-2 flex flex-wrap items-center gap-2 rounded-2xl border border-sky-300/14 bg-sky-400/8 px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] text-sky-100/72 shadow-[0_0_22px_rgba(56,189,248,0.08)]">
          <Layers3 size={12} />
          Armed Scene
      ...
      <div className="mt-3 rounded-[26px] border border-white/6 bg-[linear-gradient(180deg,rgba(255,255,255,0.045),rgba(255,255,255,0.02))] p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_14px_34px_rgba(0,0,0,0.16)]">
        <div className="mb-2 flex items-center justify-between gap-3 px-1">
          <div className="text-[9px] font-black uppercase tracking-[0.2em] text-white/30">
            Rundown Memory Bank
          </div>
          <div className="text-[9px] font-semibold uppercase tracking-[0.14em] text-white/24">
            {scenes.length} recall states
          </div>
          <div className="hidden items-center gap-1 rounded-full border border-white/8 bg-white/[0.04] px-2 py-1 text-[8px] font-black uppercase tracking-[0.14em] text-white/34 md:flex">
        ...
        <div className="flex max-h-24 flex-wrap gap-2 overflow-y-auto pr-1">
            ...
            <button
              ...
              className={`group relative overflow-hidden rounded-2xl border px-3 py-2 text-[11px] font-black uppercase tracking-[0.14em] transition-all duration-200 ${
                selectedSceneId === scene.id
                  ...
                  : "border-white/18 bg-white/[0.04] text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_12px_30px_rgba(0,0,0,0.20)]"
              }`}
            >
              {selectedSceneId === scene.id ? (
              ...
              <div className="relative flex items-center gap-2">
                <span className="flex h-5 min-w-5 items-center justify-center rounded-lg border border-white/6 bg-white/[0.04] px-1 text-[8px] font-black text-white/42 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                  {index + 1}
                </span>
          ...
          {scenes.length === 0 ? (
            <div className="flex min-h-12 w-full items-center justify-center rounded-2xl border border-dashed border-white/6 bg-white/[0.02] px-3 py-3 text-center text-[11px] font-semibold text-white/32">
