...
import { CameraSlotLiveContent } from "./ProducerRoomClientChrome";
...
import type { ProducerRoomClientViewProps } from "./ProducerRoomClientView";
import BottomAssetDock, { type RecordingStatus } from "./BottomAssetDock";
import { type ProducerWorkspaceMode } from "./ProducerModeBar";
import { type ProducerSafetyAction } from "./ProducerSafetyDialog";
import { ProducerRoomCenterColumn } from "./ProducerRoomShell";
...
export function useProducerRoomClient({
  eventId,
  sessionId,
  sessionTitle,
  eventTitle,
...
updateShadowColor: updateSelectedBlockShadowColor,
  ...
  const renderCameraSlotContent = useCallback(
    (block: PreviewBlock): ReturnType<typeof createElement> | null => {
      if (block.type !== "camera-slot" || !block.assignedParticipantId) {
        return null;
      }

      return createElement(CameraSlotLiveContent, { block });
    },
    [],
  );
  ...
  const centerColumn = createElement(
    ProducerRoomCenterColumn,
    null,
    createElement(CenterSwitcherColumn, centerSwitcherProps),
  );
...
onSaveScene: saveScene,
  ...
  const bottomDock = createElement(BottomAssetDock, bottomAssetDockProps);

  const viewProps: ProducerRoomClientViewProps = {
    token,
    serverUrl,
    liveKitRoomKey,
    setTransportHealth,
...
export default useProducerRoomClient;
