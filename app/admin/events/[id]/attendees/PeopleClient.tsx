...
import { Check, CircleCheckBig, Download, Grid3x3, Mail, Plus, RefreshCw, Search, Trash2, Upload, UserRound, Users, X } from "lucide-react"
...
export default function PeopleClient({ 
  ...
  const [importRosterMode, setImportRosterMode] = useState<MatrixMode>(initialDistricts.length ? "district" : "session")
  ...
  const [editPerson, setEditPerson] = useState({ first_name: "", last_name: "", email: "", role: "registrant" as Role, district_meeting_url: "", district_session_id: "" })
  ...
  }, [eventId, initialAttendees, initialSessions, initialDistricts])
  ...
  const districtOf = useCallback(
    (person: Person) => person.session_ids.map((id) => districtById.get(id)).find(Boolean) || null,
    [districtById]
  )
  ...
  const districtCounts = useMemo(() => {
    ...
    for (const person of people) {
      for (const sessionId of person.session_ids) {
        if (districtById.has(sessionId)) tally.set(sessionId, (tally.get(sessionId) || 0) + 1)
      }
    }
    return tally
  }, [districtById, people])
  ...
  const matrixColumns = useMemo<MatrixColumn[]>(() => matrixMode === "district"
    ...
    : sessions.filter((session) => !districtById.has(session.id)).map((session) => ({ id: session.id, code: session.code, title: session.title, subtitle: null as string | null })),
  ...
  function startEditing() {
    ...
    setEditPerson({
      ...
      role: selected.role,
      district_meeting_url: district?.external_join_url || "",
      district_session_id: district?.id || ""
    })
    setEditMode(true)
  }
  ...
  function cancelEditing() {
    setEditMode(false)
    setEditPerson({ first_name: "", last_name: "", email: "", role: "registrant", district_meeting_url: "", district_session_id: "" })
  }
    ...
    try {
      const response = await fetch(`/api/admin/events/${eventId}/attendees/${selected.id}`, {
        ...
        body: JSON.stringify({
          ...
          district_session_id: editPerson.district_session_id || null
        })
      })
      if (!response.ok) {
  ...
  const assignDistrict = useCallback(async (personIds: string[], districtSessionId: string | null, add = true) => {
    ...
    try {
      ...
      setPeople(previous)
      setError(districtError instanceof Error ? districtError.message : "Could not update district assignments")
    } finally {
      setBusy(null)
    }
  ...
  async function copyPresenterLink() {
    if (!selected || selected.session_ids.length !== 1) return
    setBusy("copy")
    setError(null)
    ...
    try {
      const response = await fetch(`/api/admin/events/${eventId}/presenters/access-link`, {
        ...
        body: JSON.stringify({
          ...
          sessionId: selected.session_ids[0],
        }),
      })
      ...
      setError(copyError instanceof Error ? copyError.message : "Could not copy presenter link")
    } finally {
      setBusy(null)
    }
  }

  const template = importRosterMode === "district"
    ? ["email,first_name,last_name,tag,notes,district_code_1,district_code_2", "jane@company.com,Jane,Doe,Registrant,,D001,D004"].join("\n")
    : ["email,first_name,last_name,tag,notes,session_code_1,session_code_2", "jane@company.com,Jane,Doe,Registrant,,KEYNOTE,WORKSHOP-A"].join("\n")
  ...
  return (
    <div className="space-y-5 text-white">
      ...
      {importOpen ? <section className="rounded-2xl border border-white/10 bg-[#0c1420]/95 p-5 shadow-[0_20px_60px_rgba(0,0,0,.28)]"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="font-semibold">Import people from CSV</div><div className="mt-1 max-w-2xl text-xs leading-5 text-white/50">Districts are specialized sessions. Choose which assignment columns your roster contains; Jupiter will attach every listed code to that person.</div><div className="mt-4 inline-flex rounded-xl border border-white/10 bg-black/20 p-1">{([{ id: "district", label: "District roster" }, { id: "session", label: "Session roster" }] as const).map((mode) => <button key={mode.id} type="button" onClick={() => setImportRosterMode(mode.id)} className={`rounded-lg px-3 py-2 text-xs font-semibold transition ${importRosterMode === mode.id ? "bg-violet-500 text-white" : "text-white/45 hover:text-white"}`}>{mode.label}</button>)}</div><p className="mt-3 max-w-2xl text-xs leading-5 text-white/45">{importRosterMode === "district" ? "Use district_code_1, district_code_2, and so on. A person may belong to multiple districts." : "Use session_code_1, session_code_2, and so on for non-district program access."}</p></div><button type="button" onClick={() => download(`${importRosterMode}_roster_${eventSlug}.csv`, template)} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.04] px-3 py-2 text-xs hover:bg-white/[.08]"><Download size={14} />Download {importRosterMode} template</button></div><div className="mt-5 flex flex-wrap items-center gap-3 rounded-xl border border-white/[.08] bg-black/20 p-3"><input ref={fileInput} type="file" accept=".csv,text/csv" className="min-w-0 flex-1 text-sm text-white/65 file:mr-3 file:rounded-lg file:border-0 file:bg-white/10 file:px-3 file:py-2 file:text-xs file:font-semibold file:text-white hover:file:bg-white/15" /><button type="button" onClick={() => void importCsv()} disabled={busy === "import"} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold shadow-[0_10px_30px_rgba(5,150,105,.18)] hover:bg-emerald-500 disabled:opacity-40">{busy === "import" ? `Uploading ${importProgress.percent}%…` : "Import people"}</button></div><OperationProgressPanel progress={importProgress} /></section> : null}
      ...
      {matrixOpen && (districts.length > 0 || sessionColumns.length > 0) ? (
        <section className="rounded-3xl border border-white/10 bg-white/[.03] p-5">
          ...
          <div className="mt-4 max-h-[560px] overflow-auto rounded-2xl border border-white/[.07]">
            <table className="w-full select-none border-separate border-spacing-0 text-sm">
              <thead>
                <tr>
