...
export default async function EventPageEditorPage(props: {
  ...
  const pages = [
    ["Event home", "Hero, countdown, featured sessions", "Published", "event_home"],
    ["Agenda", "Day and track schedule", "Published", "agenda"],
    ["Districts", "District selector and meeting links", "Published", "breakouts"],
    ["Session lobby", "Arrival and speaker information", "Published", "lobby"],
    ["Resources", "Downloads and follow-up content", "Draft", "on_demand"],
  ]
  ...
  return <div className="space-y-6 text-white">
    <header className="flex flex-col gap-5 border-b border-[#273348] pb-7 lg:flex-row lg:items-end lg:justify-between">
      ...
      <div className="flex gap-2"><Link href={`/admin/page-editor/event/${event.slug}?mode=preview&page=event_home`} target="_blank" className="rounded-xl bg-[#3974df] px-4 py-2.5 text-sm font-semibold">Preview site →</Link><Link href={`/admin/page-editor/event/${event.slug}?mode=edit&page=event_home`} className="rounded-xl bg-[#6750d3] px-4 py-2.5 text-sm font-semibold">Edit experience →</Link></div>
    </header>
    ...
    <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_382px]">
      <div className="rounded-2xl border border-[#273348] bg-[#070c16] p-5"><div className="flex items-center justify-between"><h2 className="text-xl font-semibold">Pages and moments</h2><Link href={`/admin/page-editor/event/${event.slug}?mode=edit`} className="text-xs text-[#9aa6bb]">Manage pages →</Link></div><div className="mt-4 space-y-3">{pages.map(([name, detail, status, pageKey]) => <Link key={name} href={`/admin/page-editor/event/${event.slug}?mode=edit&page=${pageKey}`} className="flex items-center justify-between gap-4 rounded-xl bg-[#1a2231] p-3 transition hover:bg-[#222c3d]"><div><div className="text-sm font-medium">{name}</div><div className="mt-1 text-xs text-[#9aa6bb]">{detail}</div></div><span className={`rounded-full border px-2 py-1 text-[9px] font-semibold uppercase tracking-wider ${status === "Published" ? "border-[#54e5a5] text-[#54e5a5]" : "border-[#68758c] text-[#9aa6bb]"}`}>● {status}</span></Link>)}</div></div>
