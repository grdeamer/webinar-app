  ...
  const [{ data: event, error: eventError }, { data: sessions, error: sessionsError }] =
    await Promise.all([
      ...
      supabaseAdmin
        .from("event_sessions")
        .select(
          "id,event_id,code,title,description,starts_at,ends_at,presenter,join_link,room_key,manual_live,playback_type,playback_mp4_url,playback_m3u8_url,sort_order,created_at,updated_at"
        )
        .eq("event_id", id)
