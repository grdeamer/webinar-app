...
export default function TeamAccessClient({ initialMembers, events, canManage }: { initialMembers: TeamMember[]; events: EventOption[]; canManage: boolean }) {
  ...
  function openEdit(member: TeamMember) {
    if (member.scope !== "event" || !member.event_role) return
    setEditingMember(member)
    setEventRole(member.event_role)
    setFeatures(featuresForEventRole(member.event_role, member.feature_permissions))
    setMenuId(null)
    setError(null)
  }

  async function saveEventAccess() {
    if (!editingMember?.event_id) return
    setBusy(true)
    setError(null)
    try {
      const response = await fetch(`/api/admin/events/${editingMember.event_id}/team/${editingMember.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ role: eventRole, featurePermissions: features }),
      })
      const payload = await response.json().catch((): null => null)
      if (!response.ok) throw new Error(payload?.error || "Could not update event access")
      setMembers((current) => current.map((member) => member.id === editingMember.id ? { ...member, event_role: eventRole, feature_permissions: features } : member))
      setNotice(`${editingMember.name || editingMember.email}’s event access was updated.`)
  ...
  return (
    <div className="global-editorial-page team-access-page mx-auto max-w-[1440px]">
      ...
      <section className="overflow-x-auto border-y border-white/10">
        <div className="min-w-[920px]">
          ...
          {members.map((member) => <div key={member.id} className="grid grid-cols-[1.35fr_.65fr_1fr_.65fr_.55fr_48px] items-center gap-4 border-b border-white/[.075] px-3 py-5 last:border-0">
            ...
            <div className="relative flex justify-end">{canManage ? <><button type="button" aria-label={`Access options for ${member.email}`} onClick={() => setMenuId((current) => current === member.id ? null : member.id)} className="rounded-lg p-2 text-white/42 hover:bg-white/[.06] hover:text-white"><DotsHorizontal className="h-4 w-4" /></button>{menuId === member.id ? <div className="absolute right-0 top-10 z-20 w-64 rounded-xl border border-white/10 bg-[#0b101d] p-1.5 shadow-2xl">{member.scope === "event" ? <button type="button" disabled={busy} onClick={() => openEdit(member)} className="w-full rounded-lg px-3 py-2 text-left text-sm text-white/70 hover:bg-white/[.06]">Edit event access</button> : null}<button type="button" disabled={busy} onClick={() => void copyTestLogin(member)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-white/70 hover:bg-white/[.06]"><Eye className="h-4 w-4 text-blue-200/70" />Copy one-time test login</button><button type="button" disabled={busy} onClick={() => { setAvatarMember(member); setMenuId(null) }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-white/70 hover:bg-white/[.06]"><Camera className="h-4 w-4 text-violet-200/70" />Change profile photo</button><div className="my-1 h-px bg-white/[.07]" /><button type="button" disabled={busy} onClick={() => void resendInvitation(member)} className="w-full rounded-lg px-3 py-2 text-left text-sm text-white/70 hover:bg-white/[.06]">{member.invite_status === "pending" ? "Send invitation" : "Send Jupiter access email"}</button>{member.invite_status !== "pending" ? <button type="button" disabled={busy} onClick={() => void sendPasswordReset(member)} className="w-full rounded-lg px-3 py-2 text-left text-sm text-white/70 hover:bg-white/[.06]">Send password reset</button> : null}{member.team_role !== "owner" ? <button type="button" disabled={busy} onClick={() => void setActive(member, !member.is_active)} className="w-full rounded-lg px-3 py-2 text-left text-sm text-white/70 hover:bg-white/[.06]">{member.is_active ? "Disable access" : "Restore access"}</button> : <div className="px-3 py-2 text-xs text-white/35">Owner access is protected</div>}</div> : null}</> : member.team_role === "owner" ? <span title="Protected account"><Lock01 className="h-4 w-4 text-white/38" /></span> : null}</div>
          </div>)}
      ...
      {editingMember ? <div role="dialog" aria-modal="true" aria-labelledby="edit-access-title" className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/75 p-5 backdrop-blur-sm"><div className="my-auto w-full max-w-2xl rounded-2xl border border-white/12 bg-[#080d19] p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><div className="text-[10px] font-semibold uppercase tracking-[.2em] text-violet-200/60">{editingMember.event_title}</div><h2 id="edit-access-title" className="mt-2 text-2xl font-semibold">Edit {editingMember.name || editingMember.email}</h2></div><button type="button" aria-label="Close" onClick={() => setEditingMember(null)} className="rounded-lg p-2 text-white/45 hover:bg-white/[.06]"><X className="h-4 w-4" /></button></div><div className="mt-5 grid grid-cols-3 gap-2">{roleOptions.map((option) => <button key={option.value} type="button" onClick={() => chooseRole(option.value)} className={`rounded-xl border px-3 py-3 text-sm font-semibold ${eventRole === option.value ? "border-violet-400/55 bg-violet-500/16" : "border-white/10 text-white/55"}`}>{option.label}</button>)}</div><div className="mt-5 grid gap-2 sm:grid-cols-2">{EVENT_FEATURES.map((feature) => <label key={feature.key} className={`flex cursor-pointer gap-3 rounded-xl border p-3 ${features.includes(feature.key) ? "border-blue-400/35 bg-blue-500/[.08]" : "border-white/[.08]"}`}><input type="checkbox" checked={features.includes(feature.key)} onChange={() => toggleFeature(feature.key)} /><span className="text-sm text-white/80">{feature.label}</span></label>)}</div>{error ? <div className="mt-4 text-sm text-red-200">{error}</div> : null}<div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => setEditingMember(null)} className="rounded-xl border border-white/10 px-4 py-3 text-sm font-semibold">Cancel</button><button type="button" disabled={busy || features.length === 0} onClick={() => void saveEventAccess()} className="rounded-xl bg-violet-600 px-5 py-3 text-sm font-semibold disabled:opacity-40">{busy ? "Saving…" : "Save access"}</button></div></div></div> : null}
